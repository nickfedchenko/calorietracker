...
final class CookingTimeBadge: UIView {
    private let timerImage: UIImageView = {
        ...
        imageView.image = R.image.timerIconWhite()
        return imageView
    }()
...
final class RecipeMainImageView: UIView {
    ...
    func setupFor(dish: Dish) {
        if let imageUrl = URL(string: dish.photo) {
            mainImage.kf.setImage(with: imageUrl)
        }
        cookingTimeBadge.setCookingTime(time: dish.cookingTime)
    }
    ...
    private func setupAppearance() {
        ...
        anotherShadowLayer.shadowOffset = CGSize(width: 0, height: 4)
        anotherShadowLayer.shadowColor = UIColor(hex: "06BBBB").cgColor
        anotherShadowLayer.shadowOpacity = 0.2
        anotherShadowLayer.shadowRadius = 10
